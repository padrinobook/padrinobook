...
- **bin**: contains the installed executables files installed by bundler. They are used to prepare the environment for
  your app to run the exact defines executables in this bin folder. This ensure that no other version then the generates is
  used when your app is deployed and run in production.
...
- **Rakefile**: Is the file to manage build automation tasks. For example will print the rake task `rake routes` all the
  defined routes in your padrino application
